app.factory('BookingService',['$http','$rootScope',function($http,$rootScope){
    var BookingServices = {};
    BookingServices.getRooms = function(callback){
    ...
    BookingServices.saveBooking = function(data,callback){
    ...
    BookingServices.checkOutRoom = function(data,callback){
        var url=$rootScope.baseUrl+ '/api/Checkouts/CheckoutAdmin';
        var d = {};
        d.requestData = data;
        $http({
            method: 'POST',
    ...
    BookingServices.extendRoom = function(data,callback){
        var url=$rootScope.baseUrl+ '/api/Extensions/ExtendAdmin';
        var d = {};
        d.requestData = data;
        $http({
            method: 'POST',
    ...
    BookingServices.cancelRoom = function(data,callback){
        var url=$rootScope.baseUrl+ '/api/Cancels/CancelAdmin';
        var d = {};
        d.requestData = data;
        $http({
            method: 'POST',
    ...
    BookingServices.getBookings = function(callback){
...
app.controller('bookingCtr', ['$http','$rootScope','BookingService','$routeParams','$scope','$timeout','$location','Upload',function ($http,$rootScope,BookingService,$routeParams,$scope,$timeout,$location,Upload) {
    $scope.inputFocused = function(type){
        $('.errorName').hide();
    };
    var getBookings = function(){

        BookingService.getBookings(function(status,bookings){
    ...
    $scope.checkOutRoom = function(booking){
        ...
        $scope.checkOut.roomId = booking.rooms.roomId;
    ...
    $scope.completeCheckout= function(){
        ...
        if(!$scope.checkOut.amountPaid < 0){
            ...
            $('#checkOutModal #cost-error,#checkOutModal .errorNameCost').addClass('error');
            $('#checkOutModal #cost-error').text(html);
            $('#checkOutModal .errorNameCost').show();
        }
        ...
        if(moment(checkInTime).isAfter(checkOutTime)){
            ...
            $('#checkOutModal #checkout-error,#checkOutModal .errorCheckout').addClass('error');
            $('#checkOutModal #checkout-error').text(html);
            $('#checkOutModal .errorCheckout').show();
            console.log('checkout is shown');
        }
        if(!valid){
        ...
        var data = {
            ...
            'roomId':$scope.checkOut.roomId
        }
        $('#checkOutModal').closeModal();
        console.log('data is '+JSON.stringify(data));
        BookingService.checkOutRoom(data,function(status){
            if(status == "SUCCESS"){
    ...
    $scope.extendBooking = function(booking){
        ...
        $scope.extend.roomId = booking.rooms.roomId;
    ...
    $scope.completeExtend= function(){
        ...
        if(moment(checkInTime).isAfter(checkOutTime)){
            ...
            $('#extensionModal #checkout-error,#extensionModal .errorCheckout').addClass('error');
            $('#extensionModal #checkout-error').text(html);
            $('#extensionModal .errorCheckout').show();
        }
        if(!valid){
            Materialize.toast('Please correct the errors',1000);
            return;
        ...
        var data = {
            ...
            'roomId':$scope.extend.roomId
        }
        $('#extensionModal').closeModal();
        console.log('data is '+JSON.stringify(data));
        BookingService.extendRoom(data,function(status){
            if(status == "SUCCESS"){
    ...
    $scope.cancelBooking = function(booking){
        ...
        $scope.cancel.roomId = booking.rooms.roomId;
    ...
    $scope.completeCancel= function(){
        ...
        if(!$scope.cancel.cancelReason.replace(/\s/g, '').length){
            ...
            $('#cancelModal #reason-error,#cancelModal .errorReason').addClass('error');
            $('#cancelModal #reason-error').text(html);
            $('#cancelModal .errorReason').show();
        }
        ...
        var data = {
            ...
            'roomId':$scope.cancel.roomId
        }
        $('#cancelModal').closeModal();
        console.log('data is '+JSON.stringify(data));
        BookingService.cancelRoom(data,function(status){
            if(status == "SUCCESS"){
...
app.controller('bookingBookCtr',['$http','$rootScope','BookingService','$scope','$timeout','$location',function ($http,$rootScope,BookingService,$scope,$timeout,$location) {
    ...
    $scope.saveBooking = function(){
        ...
        $('#bookRoomModal').closeModal();
        BookingService.saveBooking(data,function(status){
            if(status == "SUCCESS"){
    ...
    $scope.$on('bookRoom',function(event,args){
        BookingService.getRooms(function(status,rooms){
            if(status == "SUCCESS"){
                $scope.newBooking = {};
